...
if [ -e /etc/logrotate.conf ]; then
  ...
  {
    echo "  su root root"
    echo "${LOGROTATE_DIRECTORY:-/var/logs}/${LOGROTATE_FILE_PATTERN:-*.log} {"
    echo "  ${LOGROTATE_TRUNCATE:-copytruncate}"
    echo "  rotate ${LOGROTATE_ROTATE:-1}"
    echo "  size ${LOGROTATE_SIZE:-10M}"
    echo "  notifempty"
    echo "  ${LOGROTATE_COMPRESSION:-compress}"
    echo "  missingok"
    echo "  postrotate"
    echo "    /usr/bin/find ${LOGROTATE_DIRECTORY}/ -name '*.log' -type f -mtime ${LOGROTATE_DELETION_MTIME:-+1} -delete"
    echo "    /usr/bin/find ${LOGROTATE_DIRECTORY}/ -name '*.gz' -type f -mtime ${LOGROTATE_DELETION_MTIME:-+1} -delete"
    echo "  endscript"
    echo "}"
  } > /etc/logrotate.conf
fi
